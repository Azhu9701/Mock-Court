...
pub struct DomainProfile {
    ...
    /// 方法论承诺 —— 附加在综合官 system prompt 末尾的理论立场声明。
    /// 默认为哲学领域的方法论底线。设为空字符串可完全移除。
    #[serde(default = "default_methodological_commitment")]
    pub methodological_commitment: String,
}
...
impl Default for DomainProfile {
    fn default() -> Self {
        DomainProfile {
            ...
            methodological_commitment: DEFAULT_METHODOLOGICAL_COMMITMENT.to_string(),
        }
    }
}
...
fn default_methodological_commitment() -> String {
    DEFAULT_METHODOLOGICAL_COMMITMENT.to_string()
}

const DEFAULT_METHODOLOGICAL_COMMITMENT: &str = "## 方法论底线\n\n剥离比较框架的合法性。还原每个命题的物质生产条件与阶级位置——不比较\"观点\"，只比较\"谁在提取、提取什么、以谁为代价\"。承认理论立场的构成性盲区就是承认其阶级位置。将\"问题本身\"的批判指向组织化实践——不是寻找更聪明的提问方式，而是夺取定义现实的符号权力。灌输的终点不是理论共识，是被剥夺者获得行动主体性。";
...
**五步法是工具箱，不是填空题。** 根据议题的实际情况选择真正有产出的步骤。核心问题是：

- 这些魂的发言揭示了这个议题的什么？
- 哪些是一致/冲突/缺失的？
- 使用者能从中获得什么可操作的方向？

如果某个步骤确实无产出，诚实跳过——100字的真洞察胜过1000字的套路填充。简单议题不需要复杂框架。
...
### 工具性分析（适用时）
各魂的发言里，有谁指出了使用者在这个议题里被**夹在什么力量之间**？不是问\"使用者的观点对不对\"，是问：使用者被嵌入在哪两种（或多种）力量的交叉点上？他服务谁的利益，又被谁的利益压制？
...
（以下是可用板块——只输出你有实质内容的那些。没有共识就跳过共识板块，没有分歧就说没有）
...
- **需要补充**：列出 1-3 个魂名，每个附带推荐理由
- **无需补充**：直接写\"无需补充\"

**格式要求**：`- **魂名**` 必须是具体的历史/哲学人物姓名，严禁在此位置填入概念或观点摘要。";
...
\n\
以下是可用的分析层次——**根据议题复杂度选择深度。** 不是每个问题都需要走到第六层。如果你的前三层已经把问题说透了，停在第三层就是最好的深度。深度不等于篇幅：100字的真洞察胜过1000字的套路填充。\n\
\n\
