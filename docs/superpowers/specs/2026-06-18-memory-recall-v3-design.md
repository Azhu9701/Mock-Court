...
- **日期**:2026-06-18
- **状态**:**已否决(2026-06-18)**,仅作设计探索存档
- **否决理由**:经评估存在画蛇添足风险,且 v2.1 的确定收益已足够。详见下方"否决说明"
- **依赖**:v2.1 向量 hybrid 搜索 spec(`2026-06-18-vector-hybrid-search-design.md`)

---

## ❌ 否决说明(2026-06-18)

本设计探索了把 v2.1 搜索接入灵魂/综合官思维链的两条路径,经评估**不实施**,理由:

1. **B 路径(综合官注入)有污染合议风险**——综合官 synthesis 是 ThinkMax 深度推理,硬塞 top-5 历史回响可能产生锚定效应,干扰独立思考;且向量召回的 false positive(形似神不似)会带偏综合。RAG 经典陷阱:召回 ≠ 该用
2. **A 路径(灵魂工具)行为不可控**——灵魂 LLM 是否调用 recall_memory 取决于模型理解,大概率"懒得调"或"滥用调",中间态少见,投入产出比低
3. **v2.1 已覆盖确定收益**——前端 `/knowledge/search` 让**人类**能语义搜索判例,这是零风险的高价值功能;LLM 自动接入是赌不确定性,v2.1 不依赖它

**保留本文档的理由**:设计探索本身有价值(工具系统调查、prompt 注入点分析见 §4),若未来观察到"灵魂确实需要回忆""综合官确实缺历史视角"的真实痛点,可基于此文档重启。重启时应**先做最小验证**(给单个灵魂开 Memory 工具观察行为),而非一次性铺开双路径。

**当前结论**:v2.1 是终点,不是 v3 的起点。
